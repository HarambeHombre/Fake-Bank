...
import { createBankTransaction, createUserTransaction, updateBankBalance, updateTransactionStatus, createUserToUserTransaction, updateUserBalance, updateUserTransaction } from '../utils/transactionUtils';
...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const bankQuery = query(collection(db, 'users'), where('firstName', '==', 'Bank'));
  ...
  const createUser = async (userData) => {
    try {
      ...
      const initialTransaction = createUserTransaction(1000);      // Create new user in database without initial balance
      const newUserData = {
        ...userData,
